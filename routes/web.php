...
use App\Http\Controllers\AntrianObat\AntrianObat;
use App\Http\Controllers\AntrianFarmasi\AntrianFarmasiController;
use App\Http\Controllers\FormPasienRanap;
...
Route::group(['middleware' => 'default'], function () {
    ...
    Route::group(['middleware' => 'auth-rsbw'], function () {
        ...
        // ANTRIAN OBAT
        Route::get('/antrian-obat', [AntrianObat::class, 'AntrianObat']);
        Route::get('/antrian-obat-download', [AntrianObat::class, 'downloadAutorun']);
        Route::get('/panggil-obat', [AntrianObat::class, 'panggilobat']);
        Route::get('/setting-obat', [AntrianObat::class, 'settingObat']);
        Route::get('/display-farmasi', [AntrianObat::class, 'displayFarmasi']);
        ...
        //Form
        Route::get('/form-lapor-ranap', [FormPasienRanap::class, 'FormPasienRanap']);
        ...
        Route::post('/gabung-download-berkas', [BpjsController::class, 'gabungDanDownloadBerkas']);
    });
    // diplay
    ...
    Route::get('/pdf/upload', [PdfController::class, 'index'])->name('pdf.upload');
    ...
    // Route untuk halaman viewer
    Route::get('/pdf/show/{id}', function($id) {
        return view('pdf.viewer', ['id' => $id]);
    })->name('pdf.show');

    // Tambahkan route untuk proxy PDF
    Route::get('/pdf-proxy/{lokasi_file}', function ($lokasi_file) {
        try {
            $client = new \GuzzleHttp\Client();
            $response = $client->get("http://192.168.5.88/webapps/berkasrawat/pages/upload/" . $lokasi_file);

            return response($response->getBody())
                ->header('Content-Type', 'application/pdf');
        } catch (\Exception $e) {
            return response()->json(['error' => 'Tidak dapat mengakses file: ' . $e->getMessage()], 404);
        }
    })->name('pdf.proxy');

    // Perbaiki route untuk download PDF
    Route::get('/pdf-download/{no_rawat}/{index}', function ($no_rawat, $index) {
        try {
            ...
            if (count($berkas) <= $index) {
                return response()->json(['error' => 'Berkas tidak ditemukan'], 404);
            }

            $lokasi_file = $berkas[$index]->lokasi_file;
            ...
            $targetUrl = "http://192.168.5.88/webapps/berkasrawat/pages/upload/" . $cleanPath;
            ...
            return response()->json([
                'error' => 'Tidak dapat mengakses file: ' . $e->getMessage(),
                'url' => $targetUrl ?? 'URL tidak tersedia'
            ], 404);
        }
    })->name('pdf.download');

    Route::post('/show-berkas', [BpjsController::class, 'showBerkas']);
    Route::post('/simpan-berkas-scan', [BpjsController::class, 'simpanBerkasScan'])->name('simpan-berkas-scan');
